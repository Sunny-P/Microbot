...
public class CardewSlayerScript extends Script {
    ...
    AtomicBoolean isDeterminingState = new AtomicBoolean(false);
    boolean justLooted = false;

    float taskCompleteStateChangeDelayDuration = 2.0f;
    float taskCompleteStateChangeDelay = 0.0f;
    ...
    public boolean run(CardewSlayerConfig config) {
        ...
        isDeterminingState.set(false);
        justLooted = false;

        setup = new Rs2InventorySetup(config.InventorySetup(), mainScheduledFuture);
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                // Encapsulate script loop to properly make use of deltaTime
                // Yay game development principles
                MainScriptLoop(config);

                long endTime = System.currentTimeMillis();
                float totalTime = endTime - startTime;

                deltaTime = (totalTime * 0.001f);
                //Microbot.log("deltaTime: " + deltaTime);
                // If we are not doing anything deltaTime will get rounded to 0.0 incorrectly
                // So offset that by changing deltaTime to our script delay manually in these instances
    ...
    private void MainScriptLoop(CardewSlayerConfig config)
    {
        ...
        // Try to ensure we loot our final drops.
        if (taskCompleteStateChangeDelay <= 0 && currentState == States.SLAYING_MONSTER && (killsLeft <= 0))
        {
            currentState = States.MOVING_TO_NEAREST_BANK;
        }
        else if (taskCompleteStateChangeDelay > 0 && currentState == States.SLAYING_MONSTER)
        {
            taskCompleteStateChangeDelay -= deltaTime;
            Microbot.log("taskCompleteDelay: " + taskCompleteStateChangeDelay);
            return;
        }
            ...
            if (!Rs2Player.isInCombat())
            {
                ...
                else if (timeNotInCombat > 30)   // OUT OF COMBAT FOR HALF A MINUTE
                {
                    if (slayerTarget != CUtil.SlayerTarget.NONE && slayerTarget.getLocation() != null && Rs2Player.getWorldLocation() != null)
                    {
                        ...
                        {
                            if (slayerTarget == CUtil.SlayerTarget.FEVER_SPIDER) { return; }
                            Rs2Walker.walkTo(slayerTarget.getLocation());
                        }
                    }
                }
    ...
    void HandleStateMachine(CardewSlayerConfig _config)
    {
        switch(currentState)
        {
            case MOVING_TO_SLAYER_MASTER:
                ...
                if (isDeterminingState.get()) break;
            ...
            case GETTING_SLAYER_TASK:
                if (!Objects.equals(targetMonsterName, ""))
                {
                    ...
                    isDeterminingState.compareAndSet(false, true);
            ...
            case RECEIVED_TASK_RESOLVE_STATE:
                ...
                if (!isDeterminingState.get())
                {
                    isDeterminingState.set(true);
                    DetermineStateFromSlayerTask(_config);
                }
                break;

            case MOVING_TO_MONSTER_LOCATION:
                // Handle transporting with Pirate Pete
                ...
                if (!Rs2Walker.isInArea(cornerSE_MTML, cornerNW_MTML)) {
                    if (Rs2Player.getWorldLocation().getRegionID() == 8527) { return; }
                    Rs2Walker.walkWithBankedTransports(slayerTarget.getLocation());
                }
                else {
                    // We have reached our destination
                    if (slayerTarget == CUtil.SlayerTarget.FEVER_SPIDER)
                    {
                        Microbot.log("Started Fever Spider transition..");
                        Rs2Npc.interact(NpcID.DEAL_PETE, "Talk-to");
                        Microbot.log("Talked to Pirate Pete, sleeping around 2 ticks..");
                        //Rs2Dialogue.sleepUntilInDialogue();
                        sleepGaussian(1200, 100);
                        Microbot.log("Entering while loop to wait for Okay! widget");
                        while (!Rs2Dialogue.hasDialogueOption("Okay!"))
                        {
                            if (Rs2Dialogue.hasContinue())
                            {
                                Rs2Dialogue.clickContinue();
                        ...
                        //Rs2Dialogue.sleepUntilHasCombinationOption("Okay!");
                        //if (Rs2Dialogue.hasDialogueOption("Okay!"))
                        //{
                        Microbot.log("Sleeping around 1 tick..");
                        sleepGaussian(600, 200);
                        Microbot.log("Pressing Okay! widget..");
                        Rs2Dialogue.keyPressForDialogueOption("Okay!");
                        Microbot.log("Sleeping for around 8-12 seconds");
                        sleepGaussian(12000, 2000);
                        //}
                        Microbot.log("Attempting to walk near ladder..");
                        Rs2Walker.walkTo(new WorldPoint(2140, 5105, 1));
                        GameObject ladderToSpiders = Rs2GameObject.getGameObject(10168, new WorldPoint(2139, 5105, 1));
                        if (ladderToSpiders != null)
                        {
                            Microbot.log("Ladder is non-null, Climbing down..");
                            Rs2GameObject.interact(ladderToSpiders, "Climb-down");
                            sleepGaussian(3600, 200);
                        }
                    }
            ...
            case SLAYING_MONSTER:
                //if (Rs2Inventory.isFull())    // Commented out section to test new LootingParameter eatFoodForSpace property
                //{
                ...
                if (!Rs2Combat.inCombat())
                {
                    ...
                    if (!npcsInteractingWithPlayer.isEmpty())
                    {
                        ...
                        Rs2NpcModel target = npcsInteractingWithPlayer.stream()
                                ...
                                .min(Comparator.comparingInt((Rs2NpcModel npc) -> npc.getWorldLocation().distanceTo(Rs2Player.getWorldLocation()))
                                        .thenComparingDouble(npc -> Math.max(npc.getHealthPercentage(), Double.MAX_VALUE)))
                                //.stream().min(Comparator.comparingDouble(Rs2NpcModel::getHealthPercentage))
                    ...
                    else
                    {
                        ...
                        switch (slayerTarget)
                        {
                            ...
                            case MOGRE:
                                ...
                                break;
                            case MOLANISK:
                                // MOLANISK METHOD IS UNTESTED
                                DecorativeObject wallMolanisk = Rs2GameObject.getDecorativeObject(22545);
                                if (wallMolanisk != null)
                                {
                                    if (Rs2WorldPoint.quickDistance(Rs2Player.getWorldLocation(), wallMolanisk.getWorldLocation()) > 3)
                                    {
                                        Rs2Walker.walkTo(wallMolanisk.getWorldLocation(), 1);
                                    }
                                    else
                                    {
                                        // We should be near a Molanisk on a wall
                                        // Use Slayer bell to fight
                                        Rs2Inventory.interact(ItemID.MOLANISK_BELL, "Ring");
                            ...
                            case WOLF:
                                ...
                                break;
                            case WEREWOLF:
                                List<Rs2NpcModel> npcs = new ArrayList<>();
                                for (CUtil.WerewolfCanifisHumanNames name : CUtil.WerewolfCanifisHumanNames.values())
                                {
                                    //PopulateTargetList(name.getName().toLowerCase());
                                    npcs.addAll(Rs2Npc.getAttackableNpcs(name.getName()).collect(Collectors.toList()));
                                }
                                targetList = npcs;
                                //PopulateTargetList(CUtil.WerewolfCanifisHumanNames.values()[Rs2Random.between(0, CUtil.WerewolfCanifisHumanNames.values().length - 1)].ordinal());
                                break;
                            default:
            ...
            case BANKING:
                ...
                else
                {
                    ...
                    if (killsLeft > 0)
                    {
                        ...
                        {
                            /* TODO: REWRITE THIS SECTION OF BANKING, AS THIS BREAKS WITH SOME CASES THAT REQUIRE MORE THAN 1 ITEM
                                POPULATE A NEW LIST THAT IS CLEARED/NEW FOR EVERY TASK.
                                MATCH THIS LIST AGAINST THE MONSTERS REQUIRED ITEMS LIST.
                                THIS WILL ALSO ALLOW US TO ADD THESE SPECIFIC ITEMS TO BANKING EXCEPTIONS EASILY
                            */
                            ...
                            if (!requiredItemsList.get(0).equalsIgnoreCase("none") && !hasRequiredItem)
                            {
                                ...
                                boolean requireDragonfireProtection = true;
                                ...
                                for (String requiredItem : slayerTarget.getMonsterData().getItemsRequired())
                                {
                                    ...
                                    if (Rs2Bank.hasItem(requiredItem) && !requiredItem.toLowerCase().contains("bullseye lantern"))
                                    {
                                        ...
                                        if ((requiredItem.toLowerCase().contains("earmuffs") || requiredItem.toLowerCase().contains("facemask")
                                                ...
                                                && (!Rs2Bank.hasItem(ItemID.SLAYER_HELM) || !Rs2Equipment.isWearing(ItemID.SLAYER_HELM)))
                                        {
                                    ...
                                    else    // WE DO NOT HAVE THE REQUIRED ITEM IN THE BANK
                                    {
                                        ...
                                        else if (CUtil.DoNamesMatch(requiredItem, "Dragonfire shield"))
                                        {
                                            if (Rs2Equipment.isWearing(ItemID.ANTIDRAGONBREATHSHIELD))
                                            {
                                                continue;
                                            }
                                        }
                                        else if (CUtil.DoNamesMatch(requiredItem, "Antifire potion(4)"))
                                        {
                                            if (Rs2Equipment.isWearing(ItemID.ANTIDRAGONBREATHSHIELD))
                                            {
                                                continue;
        ...
        if (currentState == States.SLAYING_MONSTER || currentState == States.MOVING_TO_MONSTER_LOCATION)
        {
            switch (slayerTarget)
            {
                ...
                case BIRD:
                    ...
                    break;
                case BLUE_DRAGON:
                    AddNpcToTargetListWithValidation(_npc, _config.AlternativeBlueDragonTask().getMonsterName());
                    break;
                case DWARF:
    ...
    void IsNotWearingItemThenWithdraw(String _requiredItem, CardewSlayerConfig _config)
    {
        if (!Rs2Equipment.isWearing(_requiredItem, false))
        {
            if (Rs2Bank.hasItem(_requiredItem))
            {
                ...
                //    setup.loadInventory();
                //}
                Rs2Bank.withdrawItem(_requiredItem);
                Rs2Inventory.waitForInventoryChanges(2000);
                Rs2ItemModel withdrawnItem = Rs2Inventory.get(_requiredItem);
                for (String action : withdrawnItem.getInventoryActions())
                {
                    if (action.equalsIgnoreCase("equip") || action.equalsIgnoreCase("wear") || action.equalsIgnoreCase("wield"))
                    {
                        Rs2Inventory.wield(_requiredItem);
                        Rs2Inventory.waitForInventoryChanges(2000);
                        Rs2InventorySetup setup = new Rs2InventorySetup(_config.InventorySetup(), mainScheduledFuture);
                        setup.loadInventory();
                        break;
                ...
                if (CUtil.DoNamesMatch(potentialTarget.getMonsterData().getMonster(), targetMonsterName)) {
                    switch (potentialTarget) {
                        ...
                        case BIRD:
                            ...
                            break;
                        case BLOODVELD:
                            potentialTarget.SetLocation(_config.AlternativeBloodveldTask().getLocation());
                            break;
                        case BLUE_DRAGON:
                            potentialTarget.SetLocation(_config.AlternativeBlueDragonTask().getLocation());
                            break;
                        case CRAB:
                        ...
                        case DWARF:
                            ...
                            break;
                        case HELLHOUND:
                            potentialTarget.SetLocation(_config.AlternativeHellhoundTask().getLocation());
                            break;
                        case HILL_GIANT:
                        ...
                        case KALPHITE:
                            ...
                            break;
                        case JELLY:
                            potentialTarget.SetLocation(_config.AlternativeJellyTask().getLocation());
                            break;
                        case LESSER_DEMON:
                            potentialTarget.SetLocation(_config.AlternativeLesserDemonTask().getLocation());
                            break;
                        case SHADES:
...
//
    ...
    {
        //currentState = States.MOVING_TO_SLAYER_MASTER;
        taskCompleteStateChangeDelay = taskCompleteStateChangeDelayDuration;
        killsLeft = 0;
        hasRequiredItem = false;
    ...
    void HandleLooting(CardewSlayerConfig _config)
    {
        ...
        if (_config.PickupRunes())
        {
            ...
            runesItemParams.setEatFoodForSpace(true);
        ...
        if (_config.PickupAndBuryBones())
        {
            ...
            boneItemParams.setEatFoodForSpace(true);
            ...
            // TODO: Test if this method uses ignored item names. Otherwise reimplement.
    ...
    void HandleSoulBearer()
    {
        if (Rs2Inventory.hasItem(ItemID.ARCEUUS_SOULBEARER))
        {
            if (Rs2Inventory.hasItem("Ensouled", false))
            {
                Rs2Inventory.interact(ItemID.ARCEUUS_SOULBEARER, "Fill");
            }
        }
    }
}
